...
fit1$results[6,] # 10-fold cv results for model with all explanatory variables is nvmax=6

# RMSE and R^2 for model with Lot.Area
fit2$finalModel$xNames # confirm explanatory variables for fit2
fit2$results[6,] # 10-fold cv results for model with all explanatory variables is nvmax=6
```
...
NOTE TO PEER GRADER:
My answer slightly disagrees with the expected answer. I am sticking with my answer due to the arguments below. Please consider giving me full or partial credit if you find my arguement compeling. Otherwise, leave me a message with my grade helping me figure out where I went wrong. Thanks!

First, I confirm that my model variables `fit1` and `fit2` contain the right predictors, and then display the results for both RMSE and R^2 after 10-Fold Cross Validation. The model with the log transformation has a slightly worse RMSE value, but a better R^2 result, meaning there is about the same prediction error for both models, but the model with the log transformation has a better linear fit. From this fact alone, it may be easy to conclude that the log transformation is the better choice. However, the information from the plot and the differences/similarity in RMSE for each model suggest that the transformation does not practiaclly lead to better predictions. 
